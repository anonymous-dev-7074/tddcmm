...
track.info.requester = msg.author;
            ...
            await m.edit(isPlaylist ? `🎶 **${playlistName}** Added to **Queue** (\`${client.formatDuration(length)}\`)!` : `🎶 **${track.info.title}** Added to **Queue** (\`${this.client.formatDuration(track.info.length)}\`)!`)
...
track.info.requester = msg.author;
        ...
        await m.edit(`🎶 **${track.info.title}** Added to **Queue** (\`${client.formatDuration(track.info.length)}\`)!`).catch(() => m.edit("**❌ There was some error while adding the song!**"));
        if (res) await res.play();
}
}
