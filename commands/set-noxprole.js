...
exports.run = (client, msg, args) => {
  ...
  if (!msg.guild.roles.cache.find(r => r.name === args[0]))
    return msg.channel.send(
      `I could not find a role with that kind of name, remember it has to be capital letters if it is.`
    );

  let role =
    msg.guild.roles.cache.find(r => r.name === args[0]).id ||
  ...
  if (array.includes(role)) {
    ...
    msg.channel.send(
      "You had already added that, so I removed it from the list."
    );
    return;
  }
